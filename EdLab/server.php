...
if (isset($_POST['reg_user'])) {
  ...
  if (count($errors) == 0) {
  	...
  	header('location: Homesignup.html');
  }
}
if (isset($_POST['login_user'])) {
  ...
  if (count($errors) == 0) {
    ...
    if (mysqli_num_rows($results) == 1) {
      $_SESSION['username'] = $username;
      $_SESSION['success'] = "You are now logged in";
      header('location: Homesignin.html');
    }else {
...
if (isset($_POST['add_course'])) {
  ...
  if (empty($coursedesc)) { array_push($errors, "Description is required"); }
  $query1 = "INSERT INTO course_review (coursename,coursedesc) 
